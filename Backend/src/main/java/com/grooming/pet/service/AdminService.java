...
public class AdminService {
    ...
    public List<AdminBookingResponse> getAllBookings(String statusFilter) {
        ...
        return bookings.stream().map(booking -> {
            BookingSlot firstSlot = bookingSlotRepository.findByBooking(booking).stream().findFirst().orElse(null);
            return new AdminBookingResponse(
                    booking.getId(),
                    booking.getCustomer().getName(),
                    booking.getSpecialist().getName(),
                    booking.getSpecialist().getSpecialty(),
                    firstSlot != null ? firstSlot.getSlot().getSlotDate() : null,
                    firstSlot != null ? firstSlot.getSlot().getStartTime() : null,
