...
public class SpecialistService {
    ...
    public SpecialistProfileResponse createProfile(SpecialistProfileRequest req) {
        ...
        specialist.setCertificates(joinCertificates(req.getCertificates()));
    ...
    public SpecialistProfileResponse updateProfile(SpecialistProfileRequest req) {
        ...
        boolean nameOrSpecialtyChanged = !specialist.getName().equals(req.getName())
                || !specialist.getSpecialty().equals(req.getSpecialty());
        if (nameOrSpecialtyChanged
                && specialistRepository.existsByNameAndSpecialty(req.getName(), req.getSpecialty())) {
            throw new ConflictException("A specialist with this name and specialty combination already exists.");
        }

        String newCerts = joinCertificates(req.getCertificates());
        boolean materialChange =
                !java.util.Objects.equals(specialist.getSpecialty(), req.getSpecialty())
                || !java.util.Objects.equals(specialist.getQualificationLevel(), req.getQualificationLevel())
                || !java.util.Objects.equals(specialist.getCertificates(), newCerts);
        ...
        specialist.setBio(req.getBio());
        specialist.setPriceAmount(req.getFee());
        specialist.setCertificates(newCerts);

        if (materialChange || specialist.getStatus() == Specialist.Status.REJECTED) {
            specialist.setStatus(Specialist.Status.PENDING);
    ...
    public List<PendingSpecialistResponse> getPendingProfiles() {
        ...
        return specialistRepository.findByStatus(Specialist.Status.PENDING).stream()
                .map(s -> new PendingSpecialistResponse(
                        s.getId(), s.getName(), s.getSpecialty(),
                        s.getQualificationLevel(), s.getBio(), s.getPriceAmount(),
                        s.getCreatedAt(), splitCertificates(s.getCertificates())))
    ...
    public SpecialistProfileResponse reviewProfile(Long id, ReviewRequest req) {
        ...
        if ("APPROVE".equals(action)) {
            ...
            specialist.setRejectionReason(req.getReason().trim());
        } else {
    ...
    public List<SpecialistSummaryResponse> search(String name, String category, String level,
                                                  String date, String slotDate, String startTime, String endTime) {
        ...
        List<Specialist> results = specialistRepository.search(nameParam, categoryParam, levelParam);
        String dateParam = (slotDate != null && !slotDate.isBlank()) ? slotDate : date;
        if (dateParam != null && !dateParam.isBlank()) {
            LocalDate filterDate = LocalDate.parse(dateParam.replace('/', '-'));
            LocalTime filterStart = (startTime != null && !startTime.isBlank()) ? LocalTime.parse(startTime) : null;
            LocalTime filterEnd = (endTime != null && !endTime.isBlank()) ? LocalTime.parse(endTime) : null;
            results = results.stream()
                    .filter(s -> slotRepository.findBySpecialistAndSlotDate(s, filterDate).stream()
                            .anyMatch(slot -> slot.getStatus() == Slot.Status.AVAILABLE
                                    && !bookingSlotRepository.hasActiveBooking(slot.getId())
                                    && matchesTimeRange(slot, filterStart, filterEnd)))
                    .collect(Collectors.toList());
        }
    ...
    private boolean matchesTimeRange(Slot slot, LocalTime filterStart, LocalTime filterEnd) {
        if (filterStart == null && filterEnd == null) return true;
        if (filterStart != null && filterEnd != null) {
            return !slot.getStartTime().isBefore(filterStart) && !slot.getEndTime().isAfter(filterEnd);
        }
        if (filterStart != null) return !slot.getStartTime().isBefore(filterStart);
        return !slot.getEndTime().isAfter(filterEnd);
    }
    ...
    private SpecialistProfileResponse toProfileResponse(Specialist s) {
        return new SpecialistProfileResponse(
                ...
                splitCertificates(s.getCertificates()), s.getRejectionReason());
    }

    private String joinCertificates(List<String> certs) {
        if (certs == null || certs.isEmpty()) return null;
        return String.join(",", certs);
    }

    private List<String> splitCertificates(String certs) {
        if (certs == null || certs.isBlank()) return Collections.emptyList();
        return Arrays.asList(certs.split(","));
    }
}
